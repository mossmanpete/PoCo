...
contract('IexecHub', function(accounts) {
	...
	before("should prepare accounts and check TestRPC Mode", async() => {
		...
		txsMined = await Promise.all([
			...
			aRLCInstance.transfer(web3jScheduler,   1000, { from: marketplaceCreator, gas: amountGazProvided }),
			aRLCInstance.transfer(web3jWorker1,   	1000, { from: marketplaceCreator, gas: amountGazProvided }),
			aRLCInstance.transfer(web3jWorker2,   	1000, { from: marketplaceCreator, gas: amountGazProvided }),
		]);
		...
		assert.isBelow(txsMined[7].receipt.gasUsed, amountGazProvided, "should not use all gas");
		assert.isBelow(txsMined[8].receipt.gasUsed, amountGazProvided, "should not use all gas");
		assert.isBelow(txsMined[9].receipt.gasUsed, amountGazProvided, "should not use all gas");
